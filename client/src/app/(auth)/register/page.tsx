...
import { Mail, Lock, ArrowRight, AlertCircle, ChevronLeft, User, Eye, EyeOff } from "lucide-react";
    ...
    <div className="h-screen flex bg-white text-slate-900 font-sans overflow-hidden">
      ...
      <div className="hidden lg:flex w-1/2 bg-slate-900 relative flex-col justify-between p-12 text-white">
        <div className="absolute inset-0 z-0 opacity-20">
             <div className="absolute top-0 right-0 w-[800px] h-[800px] bg-violet-600 rounded-full blur-[120px] translate-x-1/3 -translate-y-1/2"></div>
             <div className="absolute bottom-0 left-0 w-[600px] h-[600px] bg-indigo-600 rounded-full blur-[100px] -translate-x-1/3 translate-y-1/3"></div>
        ...
        <div className="relative z-10 max-w-lg">
            <h2 className="text-3xl font-bold leading-tight mb-4">
                Join thousands of brands <br/> growing with smarter links.
            </h2>
            <ul className="space-y-2 text-slate-400 text-sm">
                <li className="flex items-center gap-2">
                    <span className="text-emerald-400">✓</span> Unlimited Dynamic QR Codes
                </li>
                <li className="flex items-center gap-2">
                    <span className="text-emerald-400">✓</span> Real-time Scan Analytics
                </li>
            </ul>
        </div>
      ...
      <div className="w-full lg:w-1/2 flex flex-col items-center justify-center p-8 relative bg-white h-full">
        ...
        <div className="w-full max-w-[400px]">
            ...
            <div className="mb-5 text-center lg:text-left">
                <h1 className="text-2xl font-bold text-slate-900 tracking-tight mb-1">Create Account</h1>
            ...
            <form onSubmit={handleRegister} className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1">
                        ...
                        <div className="relative group">
                            ...
                            <input
                                ...
                                required
                                className="block w-full pl-10 pr-8 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 placeholder:text-slate-400 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition text-sm font-medium"
                                placeholder="8+ chars"
                                value={password}
                                onChange={(e) => setPassword(e.target.value)}
                            />
                            <button type="button" onClick={() => setShowPass(!showPass)} className="absolute inset-y-0 right-0 pr-2 flex items-center text-slate-400 hover:text-slate-600 cursor-pointer">
                    ...
                    <div className="space-y-1">
                        <label className="text-xs font-semibold text-slate-900 uppercase tracking-wide">Confirm</label>
                        <div className="relative group">
                            <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-slate-400 group-focus-within:text-indigo-600 transition">
                                <Lock size={18} />
                            </div>
                            ...
                            />
                             <button type="button" onClick={() => setShowConfirm(!showConfirm)} className="absolute inset-y-0 right-0 pr-2 flex items-center text-slate-400 hover:text-slate-600 cursor-pointer">
                ...
                <button
                    type="submit"
                    disabled={loading}
                    className="w-full bg-indigo-600 text-white py-3 rounded-xl font-bold hover:bg-indigo-700 focus:ring-4 focus:ring-indigo-500/20 transition shadow-lg flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed text-sm mt-2"
                >
                    {loading ? (
            ...
            <div className="relative my-5">
                ...
                <div className="relative flex justify-center text-[10px] uppercase tracking-wide">
                    <span className="px-3 bg-white text-slate-400 font-bold">Or register with</span>
                </div>
            </div>

            <div className="grid grid-cols-3 gap-3">
                <button className="flex justify-center items-center py-2 border border-slate-200 rounded-xl hover:bg-slate-50 transition hover:border-slate-300">
                ...
                <button className="flex justify-center items-center py-2 border border-slate-200 rounded-xl hover:bg-slate-50 transition hover:border-slate-300 text-slate-600">
            ...
            <p className="mt-5 text-center text-sm text-slate-500">
                ...
                <Link href="/login" className="font-bold text-indigo-600 hover:text-indigo-700 hover:underline transition">
                    Sign in
                </Link>
            </p>
        </div>
